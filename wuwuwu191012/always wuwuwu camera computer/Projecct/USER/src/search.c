...
float Turn_P_Cam=2; 
float Turn_D_Cam=1.5;
float old_Cam_offset = 0;
float Cam_offset = 0 ;
...
/**
camera_main funcs
      Pic_seedfill();//种子搜索算法 g51-93 ->Pic_seedfill_grow()//递归 g95-127
      Pic_noi_elim();//图像简单去噪点 g138-159
      Pic_DrawMid();//寻找左右边线  g173-269
      Cam_End_Detect();//End记录从上至下最后一行边界行
      Pic_undistort();//图像去畸变 -> pixel_undistort()//像素去畸变
...
void Pic_seedfill(void){
	...
	//seed_x 图像DFS搜索算法起始搜索区域 defualt (30,40)~(50,55)
        
	//设定基准团(点)
	for(i=seed_down;i>=seed_up;i-=3){
		for(j=seed_left;j<=seed_right;j+=3){
			if(Pixle[i][j]==1){
				seed_flag=1; 
				seed_i=i;
				seed_j=j;
...
void Pic_DrawMid(void)
{
  ...
  {
    Rig[i]=78; //第i行的右边线在第Rig[i]列
    Lef[i]=1;  //第i行的左边线在第Lef[i]列
    Mid[i]=39; //第i行的中间线在第Mid[i]列
    for(j=0;j<Fir_col;++j)
    {
      Pixle[i][j]=0; //左侧置黑
    }
...
//    }
  ...
  for(j=Middle;j<Last_col+1;++j)//末行左右边线计算
  {
  ...
  for(i=Last_row-1;i>Fir_row-1;--i)//从底层向上绘制左右边线
  {
    Lef_err[i]=Rig_err[i]=0;
    for(j=Rig[i+1]-10;j<Last_col+1;++j)//向右搜索下一跳变点
    {
      ...
      if(j==Last_col)//若没有找到跳变点，则放宽范围进行搜索
      {
        ...
        for(k=Lef[i+1]+5;k<Rig[i+1]-10;++k)
        {
          ...
          {
            Rig[i]=k+2;//放宽之后的右边线 +2？？
            break;
          }          
        }
      }
    }
    for(j=Lef[i+1]+10;j>Fir_col-1;--j)//向左搜索下一跳变点
    {
...
*  修改时间：2019.3.21
*  备    注：对Lef、Rig进行映射处理 
             MIDMAP应该是去畸图像的长度一半，中间为0，从-MIDMAP到MIDMAP
             现在define的是300，也就是六百列
             offsetmap最大值31/9，temx最大值79，算出来最大1300多？             
...
int temx; //不知为何放在外面
int mid_map[60]; //定义之后再没用过
//my analitical version of remapping x and y
void pixel_undistort(int x,int y,int LR)//LR:Left(0) or Right(1)
{
  temx=x;
  if(temx==2)temx=0;
  if(temx==78)temx=79;
  if(LR==0){
	New_Lef[y]=(int)(10*(temx-39)*offset_map[y]);//某种坐标转换
        if(New_Lef[y]<-MIDMAP) 
          New_Lef[y]=-MIDMAP;
  }else{
...
void Cam_End_Detect(void)  
{
  ...
  Cam_End = max_block(Lef_End,Rig_End);//max_block 取较大值
}
...
*  备    注：最小二乘法拟合FIG_AREA区域内不为0的斜率
             y = b0 + b1*x
             b0 = sum(y)/count - b1 * sm(x)/count
             b1 = sum(xy) - count * sum(x)/count * sum(y)/count
                  ---------------------------------------------
                  sum(x^2) - count * (sum(x)/count)^2
                = sum(xy) * count - sum(x) * sum(y)
                  ---------------------------------------------
                  sum(x^2) * count - (sum(x))^2
...
void LR_Slope_fig()
{
  ...
  int max=-800,min=0;//在左边平面，-800？~0，遍历初始值设为边界
  for(i=0;i<60;i++)
  {
    if (i<=FIG_AREA_NEAR && i>=FIG_AREA_FAR3 && New_Lef[i]!=-MIDMAP)
    {
      ...
      xsum+=New_Lef[i];//x1+x2+...
      ysum+=i;//y1+y2+...
      xysum+=New_Lef[i]*i;//x1y1+x2y2+...
      x2sum+=New_Lef[i]*New_Lef[i];//x1^2+x2^2+...
      count++;
    }
  }
  //calculate 
  if(abs(max-min)>50)
  {
      if(count*x2sum-xsum*xsum)//分母不为0
      {
  ...
  LR_slope=L_R_Slope(1)+L_R_Slope(2);//左边线 右边线 只用于判断障碍物 使用去畸前边线
}
...
float Slope(int F1x,int F1y,int F2x,int F2y)//可以只传入Dy，少一个参数？
{
  float slope=0;
 // if(F2y==F1y)
  if(F2x==F1x)
  {
    return 999;//这个咋回事 错了吧 先改成F2x == F1x 了
  }
...
float L_R_Slope(char LR)
{
  ...
  if(LR == 1)
  {
    for(i=5;i<20;i++)
    {
       if(Lef[i]!=Fir_row)
       {
         slope[row++]=Slope(Lef[i],i,Lef[i+10],i+10); //和10行后算斜率，去畸变前
       }
    }
    for(j=row-1;j>=0;j--)
    {
      ...
      sum/=(row-1);//平均斜率 //如果row-1 = 0？？是否需要判断
      return sum;
    }
...
{  
  ...
  for(i=Fir_row;i<Last_row+1;i++)//全白为1，反之0
  {
    Allwhiterow[i]=0;
...
void Pic_find_leap(void)
{
  ...
  {
    if(abs(Lef[i]-Lef[i-2])<5 && abs(Lef[i-4]-Lef[i-2])>6)//！！条件可能偏宽松 //！！我觉得学长你说得很对
    {
      Lef_leap[count1]=i-3;
      count1++;
...
//  {
//    Road0_flag = 0;
//    if(((Lef_circle==0||( Lef_circle ==1 && Lef_break_point>30)))/* && Rig_slope>=10)*/ && Road1_turnin(EM_Value_2,EM_Value_3,3.8))//Rig_slope<1 && (Lef_leap[0]==0||Lef_slope==999)&& Rig_leap[0]==0)
//    {
//      Road13_count++;
//      if(Road13_count==2)
//      {
